/*
    Konfigurasi konstruktor default:
        autoplay: false,
        shuffle: false,
        loop: false,
    ...
    Dapat mengakses player dengan variabel .player
     contoh playlist.player.pause ();
*/
...
class AudioPlaylist {
  ...
  setTrack(arrayPos) {
    ...
    var liPos = this.trackOrder[arrayPos]; // mengubah indeks array menjadi indeks html
    ...
    this.trackPos = arrayPos; // perbarui berdasarkan posisi indeks larik
  }
  prevTrack() {
    if (this.trackPos == 0)
      this.setTrack(0);
  ...
  nextTrack() {
    // jika trek bukan trek terakhir dalam larik trek, lanjutkan ke trek berikutnya
  ...
  setShuffle(val) {
    if (val == this.shuffle) // jika tidak ada perubahan
      return val;
    else {
      if (val === true) {
        this.randomizeOrder();
        ...
        this.shuffle = false;
        // larik trek kosong, isi larik dengan indeks secara berurutan
        this.trackOrder = [];
        for (var i = 0; i < this.length; i++) {
          this.trackOrder.push(i);
        }

        // lompat array untuk melacak posisi trek yang sedang diputar
  ...
  constructor(config = {}) {
    ...
    /***
     *
     *       mengatur default, dan menginisialisasi player
     *
     */

    var classObj = this; // menyimpan lingkup untuk event listeners
    ...
    /***
     *
     *       menangani klik tautan
     *
     */

    $("#" + this.playlistId + " li a ").click(function (e) {
      e.preventDefault();
      // mengatur jalur berdasarkan indeks
